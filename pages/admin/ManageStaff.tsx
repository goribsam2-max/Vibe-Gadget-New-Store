    ...
    <div className="max-w-6xl mx-auto px-6 py-10 pb-32 min-h-screen bg-zinc-50 dark:bg-zinc-800/50">
      <div className="flex items-center justify-between mb-12">
        ...
        <button
          ...
          className="bg-zinc-900 dark:bg-zinc-100 text-white px-6 py-3 rounded-full text-[10px] font-bold tracking-normal  hover:bg-emerald-900 transition-colors shadow-lg"
        >
          <Icon name={showAdd ? "times" : "plus"} className="mr-2" />{" "}
      ...
      {showAdd && (
        <motion.div
          ...
          className="bg-zinc-50 dark:bg-zinc-800 p-8 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700 mb-8 max-w-2xl"
        >
          <h3 className="font-bold text-zinc-900 dark:text-zinc-100 mb-6  tracking-normal text-xs">
            Configure Staff Access
          </h3>
          <form onSubmit={handleAddStaff}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <label className="block text-[10px] font-bold text-zinc-400  tracking-normal mb-2">
                  Staff Email
                </label>
                <input
                  type="email"
                  ...
                  className="w-full p-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl text-sm font-bold"
                  required
                />
              </div>
              <div>
            ...
            <div className="mb-8">
              <label className="block text-[10px] font-bold text-zinc-400  tracking-normal mb-4">
                Module Permissions
              </label>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {ALL_MODULES.map((mod) => (
                  <div
                    ...
                    className={`p-3 rounded-2xl border flex items-center space-x-3 cursor-pointer transition-all ${form.permissions.includes(mod.id) ? "bg-zinc-50 dark:bg-zinc-900 border-emerald-200 shadow-sm" : "bg-zinc-50 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 hover:border-zinc-300"}`}
                  >
                    <div
                      className={`w-4 h-4 rounded border flex items-center justify-center ${form.permissions.includes(mod.id) ? "bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 border-emerald-600 text-white" : "bg-zinc-50 dark:bg-zinc-800 border-zinc-300"}`}
                    >
                    ...
                    <span
                      className={`text-xs font-bold ${form.permissions.includes(mod.id) ? "text-emerald-900" : "text-zinc-600 dark:text-zinc-400"}`}
                    >
                      {mod.name}
                    </span>
                  </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="px-8 py-3 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-white rounded-full font-bold  tracking-normal text-xs hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20 disabled:opacity-50"
              >
                Confirm & Add User
              </button>
            </div>
