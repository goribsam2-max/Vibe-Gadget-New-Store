  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 pb-48 min-h-screen bg-zinc-50 dark:bg-zinc-800 font-inter">
      ...
      <button
        disabled={saving}
        onClick={handleSave}
        className="w-full py-5 bg-primary-500 text-white rounded-full font-bold text-sm  tracking-normal transition-all disabled:opacity-50"
      >
...
const AccordionSection = ({
  ...
  <div className="border border-zinc-100 dark:border-zinc-800 rounded-full overflow-hidden bg-zinc-50 dark:bg-zinc-800">
    <button
      onClick={onToggle}
      className="w-full px-6 py-5 flex items-center justify-between hover:bg-zinc-50 dark:bg-zinc-800 transition-colors"
    >
...
const Field = ({ label, value, onChange, placeholder }: any) => (
  <div>
    ...
    <input
      ...
      className="w-full bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl text-sm font-medium outline-none border border-transparent focus:border-zinc-900 transition-all"
    />
  </div>
);
