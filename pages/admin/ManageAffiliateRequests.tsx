    ...
    <div className="max-w-[1440px] mx-auto px-6 py-10 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {requests.map((req) => (
          <div
            key={req.id}
            className="bg-zinc-50 dark:bg-zinc-800 p-6 md:p-8 rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-sm flex flex-col md:flex-row gap-6"
          >
            <div className="flex-1">
              <div className="flex items-center justify-between mb-4">
                <span
                  className={`px-3 py-1 rounded-full text-[10px] font-semibold  tracking-normal ${req.status === "pending" ? "bg-yellow-100 text-yellow-700" : req.status === "approved" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                >
                  {req.status}
                </span>
              ...
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-4 tracking-tight">
                {req.fullName}
              </h3>
              <div className="space-y-3 mb-6 bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                <p className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                  <Icon name="envelope" className="text-zinc-400" />{" "}
                  <span className="text-black dark:text-white">
                    {req.email}
                  </span>
                </p>
                <p className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                  <Icon name="phone" className="text-zinc-400" />{" "}
                  <span className="text-black dark:text-white">
                    {req.phone}
                  </span>
                </p>
                {req.socialUrl && (
                  <p className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                    <Icon name="link" className="text-zinc-400" />{" "}
                    <a
                      href={req.socialUrl}
                      target="_blank"
                      rel="noreferrer"
            ...
            {req.status === "pending" && (
              <div className="flex flex-col gap-3 justify-center border-t md:border-t-0 md:border-l border-zinc-200 dark:border-zinc-700 pt-6 md:pt-0 md:pl-6">
                <button
                  ...
                  className="w-full bg-zinc-900 dark:bg-zinc-100 text-white text-[10px] font-semibold tracking-normal py-4 px-8 rounded-full  hover:bg-emerald-800 transition-colors shadow-lg active:scale-95"
                >
                  Approve
                </button>
                <button
                  onClick={() => updateStatus(req, "rejected")}
                  className="w-full bg-red-50 text-red-600 text-[10px] font-semibold tracking-normal py-4 px-8 rounded-full  hover:bg-red-100 transition-colors active:scale-95"
                >
                  Reject
                </button>
              </div>
        ...
        {requests.length === 0 && (
          <div className="col-span-full py-20 text-center text-zinc-400 font-bold  tracking-normal bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700">
