      ...
      <div className="space-y-4">
        {loading
          ...
          : orders.map((order) => (
              <div
                key={order.id}
                className="bg-white dark:bg-[#121212] rounded-full border border-zinc-100 dark:border-zinc-800/80 shadow-[0_2px_10px_rgba(0,0,0,0.02)] transition-shadow hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col xl:flex-row group"
              >
                ...
                <div className="xl:w-[320px] shrink-0 p-6 xl:p-8 bg-zinc-50/50 dark:bg-zinc-800/20 border-b xl:border-b-0 xl:border-r border-zinc-100 dark:border-zinc-800/80 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-8">
                      <p className="font-mono text-[10px] font-semibold text-zinc-400  tracking-normal">
                        #{order.id.slice(0, 10)}
                      </p>
                      <span
                        className={`px-2.5 py-1 rounded-full text-[9px] font-bold  tracking-normal ${order.status === OrderStatus.DELIVERED ? "bg-emerald-100/50 text-zinc-800 dark:text-zinc-200" : "bg-white dark:bg-zinc-800 text-zinc-500 shadow-sm border border-zinc-100 dark:border-zinc-800"}`}
                      >
                        {order.status}
                      </span>
                    </div>
                    <div className="space-y-1.5 mb-8">
                      ...
                      <p className="text-[11px] text-zinc-400 font-medium leading-relaxed mt-2 max-w-[280px]">
                        {order.shippingAddress}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-semibold text-zinc-400  tracking-normal">
                      Order Status
                    </label>
                    <div className="relative">
                      <select
                        className="w-full appearance-none p-3.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700/50 text-xs font-semibold outline-none cursor-pointer focus:border-zinc-400 dark:focus:border-zinc-500 rounded-2xl transition-all shadow-[0_2px_10px_rgba(0,0,0,0.02)]"
                        value={order.status}
                        onChange={(e) =>
                      ...
                      <Icon
                        name="chevron-down"
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-zinc-400 pointer-events-none"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 p-6 xl:p-8 flex flex-col justify-between">
                  <div>
                    <div className="flex flex-wrap gap-4 mb-8">
                      {order.items.map((item, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-4 bg-zinc-50/50 dark:bg-zinc-800/30 p-2.5 pr-5 rounded-2xl border border-zinc-100 dark:border-zinc-800/50"
                        >
                          <div className="w-12 h-12 rounded-full bg-white dark:bg-zinc-900 p-1.5 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                            <img
                              src={item.image}
                          ...
                          <div>
                            <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-200 max-w-[140px] truncate">
                              {item.name}
                            </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                    <div>
                      <label className="text-[10px] font-semibold text-zinc-400  tracking-normal block mb-2">
                        Tracking ID
                      </label>
                      <input
                        type="text"
                        placeholder="Enter Tracking ID..."
                        className="w-full bg-transparent p-3 text-xs font-semibold outline-none border-b border-zinc-200 dark:border-zinc-800 focus:border-zinc-900 dark:focus:border-zinc-400 transition-colors  placeholder:text-zinc-300 dark:placeholder:text-zinc-700"
                    ...
                    {order.status === OrderStatus.ON_THE_WAY && (
                      <div className="grid grid-cols-2 gap-6 animate-fade-in">
                        <div>
                          <label className="text-[10px] font-semibold text-zinc-800 dark:text-zinc-200 dark:text-zinc-900 dark:text-zinc-100/80  tracking-normal block mb-2">
                            Rider Number
                          </label>
                          <input
                            type="text"
                            ...
                            onBlur={(e) =>
                              updateDoc(doc(db, "orders", order.id), {
                                riderNumber: e.target.value.trim(),
                              })
                            }
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onBlur={(e) =>
                              updateDoc(doc(db, "orders", order.id), {
                                courierName: e.target.value.trim(),
                              })
                            }
                          />
                        </div>
                ...
                <div className="xl:w-[200px] p-6 xl:p-8 bg-zinc-50/80 dark:bg-zinc-800/10 border-t xl:border-t-0 xl:border-l border-zinc-100 dark:border-zinc-800/80 flex flex-col justify-between items-start xl:items-end text-left xl:text-right">
                  <div className="w-full">
                    <p className="text-[10px] font-semibold text-zinc-400  tracking-normal mb-2 xl:text-right">
                      Total Amount
                    </p>
                    {!!order.discount && (
                      <>
                        <p className="text-[10px] font-bold text-zinc-400 xl:text-right line-through">
                          ৳{(order.subTotal || 0) + 150}
                        </p>
                        <p className="text-[10px] font-bold text-zinc-900 dark:text-zinc-100 xl:text-right mb-1">
                          -৳{order.discount} ({order.couponCode})
                        </p>
                      </>
                    )}
                    <p className="text-lg xl:text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 xl:text-right">
                      ৳{order.total}
                    </p>
                  </div>
