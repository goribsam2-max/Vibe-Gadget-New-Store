    ...
    <div className="max-w-[1440px] mx-auto px-6 py-10 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {requests.map((req) => (
          <div
            key={req.id}
            className="bg-zinc-50 dark:bg-zinc-800 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-700"
          >
            <div className="flex items-center justify-between mb-4">
              <span
                className={`px-2 py-1 rounded-md text-[10px] font-bold  ${req.status === "Pending" ? "bg-yellow-100 text-yellow-700" : req.status === "Completed" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
              >
                {req.status}
              </span>
            ...
            {req.status === "Pending" && (
              <div className="flex space-x-2">
                <button
                  ...
                  className="flex-1 bg-primary-500 text-white text-[10px] font-bold py-2 rounded-lg "
                >
                  Complete
                </button>
                <button
                  onClick={() => updateStatus(req.id, "Rejected")}
                  className="flex-1 bg-red-50 text-red-600 text-[10px] font-bold py-2 rounded-lg "
                >
                  Reject
                </button>
              </div>
